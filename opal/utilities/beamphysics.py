...
def evolveBetaFunction(ls, ks, beta0, alpha0=0, fast=False):
    if not fast:
        Nres = 50
...
def evolveDispersion(ls, inv_rhos, ks, Dx0=0, Dpx0=0, fast=False):
    if not fast:
        Nres = 100
        evolution = np.empty([3, Nres*len(ls)])
    else:
...
def evolveSecondOrderDispersion(ls, inv_rhos, ks, ms, taus, fast=False):
