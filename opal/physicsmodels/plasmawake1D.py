...
def wakefield1D(n0, driver, beam=None):
    ...
    if beam is not None:
        ...
        bins = beam.offsetZ() + 5*np.linspace(-1, 1, int(np.sqrt(beam.Npart())))*beam.bunchLength()
        dN_dz, zs_beam = beam.longitudinalNumberDensity(bins=bins)
