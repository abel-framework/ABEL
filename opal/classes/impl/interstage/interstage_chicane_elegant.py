...
class InterstageChicaneELEGANT(Interstage):
    ...
    def driftLength(self):
        return (self.dipoleLength() + self.lensLength()/2) - 2*self.chicaneDipoleLength() - self.sextupoleLength()/2
       
    # full lattice 
    def fullLattice(self, g_lens=0, B_chic1=0, B_chic2=0, m_sext=0, tau_lens=0):
        ...
        ls = np.array([dL, L_dip, dL, L_lens, dL, L_chic, L_drift, L_chic, dL, L_sext, dL, L_chic, L_drift, L_chic, dL, L_lens, dL, L_dip, dL])
        
        # bending strength array
        inv_rhos = np.array([0, self.dipoleField(), 0, 0, 0, B_chic1, 0, B_chic2, 0, 0, 0, B_chic2, 0, B_chic1, 0, 0, 0, self.dipoleField(), 0]) * SI.c / self.E0
    ...
    def halfLattice(self, g_lens=0, B_chic1=0, B_chic2=0, m_sext=0, tau_lens=0):
        ls, inv_rhos, ks, ms, taus = self.fullLattice(g_lens, B_chic1, B_chic2, m_sext, tau_lens)
    ...
    def quarterLattice(self, g_lens=0, B_chic1=0, B_chic2=0, m_sext=0, tau_lens=0):
        ls, inv_rhos, ks, ms, taus = self.fullLattice(g_lens, B_chic1, B_chic2, m_sext, tau_lens)
    ...
    def makeLattice(self, beam, output_filename, latticefile, lensfile, dumpBeams=False):
        ...
        g_lens, tau_lens, B_chic1, B_chic2, m_sext = self.match()
        ...
        lattice_template = CONFIG.opal_path + 'opal/apis/elegant/templates/lattice_interstage_chicane.lte'
        Ndumps = 1
        ...
        inputs = {'charge': abs(beam.charge()),
                  ...
                  'chicanedipole_angle1': self.chicaneDipoleLength()*B_chic1*SI.c/self.E0 / Ndumps,
                  'chicanedipole_angle2': self.chicaneDipoleLength()*B_chic2*SI.c/self.E0 / Ndumps,
    ...
    def match(self):
        ...
        inv_rhos_half = lambda B1, B2: np.array([0, self.Bdip, 0, 0, 0, B1, 0, B2, 0, 0]) * SI.c/self.E0
        ...
        result_beta = sciopt.minimize(minfun_beta, self.g_max, tol=1e-20)
        g_lens = result_beta.x[0]
        
        # TODO: simultaneously match dispersion and R56
        
        # minimizer function for first-order dispersion (central dispersion prime is zero)
        def minfun_Dx(p):
            _, Dpx, _ = evolveDispersion(ls_half, inv_rhos_half(p[0], p[1]), ks_half(g_lens), fast=True)         
            return Dpx**2
        
        # match the first-order dispersion
        result_Dx = sciopt.minimize(minfun_Dx, [0, 0], tol=1e-20)
        B_chic1 = result_Dx.x[0]
        B_chic2 = result_Dx.x[1]
        ...
        def minfun_DDx(p):
            _, DDpx, _ = evolveSecondOrderDispersion(ls_half, inv_rhos_half(B_chic1, B_chic2), ks_half(g_lens), ms_half(p[0]), taus_half(tau_lens), fast=True)
            return DDpx**2
        
        # match the second-order dispersion
        m_guess = 4*tau_lens/self.sextupoleLength()
        result_DDx = sciopt.minimize(minfun_DDx, m_guess, method='Nelder-Mead', tol=1e-20, options={'maxiter': 50})
        ...
        return g_lens, tau_lens, B_chic1, B_chic2, m_sext
