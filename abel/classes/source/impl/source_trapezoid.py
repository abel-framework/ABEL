...
class SourceTrapezoid(Source):
    ...
    def track(self, _ = None, savedepth=0, runnable=None, verbose=False):
        ...
        if self.front_heavy_distribution:
            ...
            weightings[mask_uniform] = weightings[mask_uniform]/(np.sum(weightings[mask_uniform])/(self.charge/SI.e * np.sign(self.charge)) )*np.sum(mask_uniform)/len(zs)
            weightings[mask_triangle] = (1+front_heaviness*zs_norm_redist[mask_triangle])**3
            weightings[mask_triangle] = weightings[mask_triangle]/(np.sum(weightings[mask_triangle])/(self.charge/SI.e * np.sign(self.charge)) )*np.sum(mask_triangle)/len(zs)
