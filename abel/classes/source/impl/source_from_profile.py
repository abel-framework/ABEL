...
class SourceFromProfile(Source):
    
    def __init__(self, file_path: str, length=0, num_particles=1000, energy=None, charge=0, rel_energy_spread=None, energy_spread=None, z_offset=0, x_offset=0, y_offset=0, x_angle=0, y_angle=0, emit_nx=0, emit_ny=0, beta_x=None, beta_y=None, alpha_x=0, alpha_y=0, angular_momentum=0, wallplug_efficiency=1, accel_gradient=None, symmetrize=False, z_cutoff=None):
        ...
        self.file = file_path
        self.file_type = re.search(r'\.([a-zA-Z0-9]+)$', file_path).group(1)
    ...
    def track(self, _=None, savedepth=0, runnable=None, verbose=False):
        ...
        file_type_to_function = {
            ...
            "pkl": "read_pickle"}
        # The pandas read-function depending on the filetype, feel free to add to
        read_function = getattr(pd, file_type_to_function.get(self.file_type)) 
        """
        File must contain to arrays where z is first array, and Is is second array.
        Given in meters and Ampere respectively.
        """
