...
class IonMotionConfig():
    ...
    def __init__(self, drive_beam, main_beam, plasma_ion_density, ion_charge_num=1.0, ion_mass=None, num_z_cells=None, num_xy_cells_rft=50, num_xy_cells_extract=41, update_factor=1.0):
        ...
        self.xs_extract = np.linspace( xy_min, xy_max, num_xy_cells_extract)  # [m]
        ...
        self.zs_extract = np.linspace(z_max, z_min, num_z_cells)  # [m], beam head facing start of array.
        self.grid_size_z = (self.zs_extract.max() - self.zs_extract.min()) / (num_z_cells-1)  # [m], the grid size along z
...
def ion_wakefield_perturbation(ion_motion_config, sc_fields_obj, tr_direction):
    ...
    E_fields_comp_3d = E_fields_comp.reshape(len(xs_extract), len(ys_extract), len(zs_extract))  # TODO: check this array
